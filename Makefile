...
DOCKER_WORKDIR=/proj
DOCKER_RUN=docker run --rm --env CGO_ENABLED=0 -v "$$PWD/.:${DOCKER_WORKDIR}" -v "`go env GOPATH`/pkg/mod/.:/go/pkg/mod:ro" -w ${DOCKER_WORKDIR}
DOCKER_GO_BUILD=go build -mod=readonly -ldflags "$$LD_FLAGS"
...
build-docker-amd64: require-version
	...
	cd ${DOCKER_DIR} && \
		...
		-t ghcr.io/unafraid/gotify/server:latest \
		-t ghcr.io/unafraid/gotify/server:${VERSION} \
		-t ghcr.io/unafraid/gotify/server:$(shell echo $(VERSION) | cut -d '.' -f -2) \
		-t ghcr.io/unafraid/gotify/server:$(shell echo $(VERSION) | cut -d '.' -f -1) .
...
build-docker-arm-7: require-version
	...
	cd ${DOCKER_DIR} && \
		...
		-t ghcr.io/unafraid/gotify/server-arm7:latest \
		-t ghcr.io/unafraid/gotify/server-arm7:${VERSION} \
		-t ghcr.io/unafraid/gotify/server-arm7:$(shell echo $(VERSION) | cut -d '.' -f -2) \
		-t ghcr.io/unafraid/gotify/server-arm7:$(shell echo $(VERSION) | cut -d '.' -f -1) .
	rm ${DOCKER_DIR}gotify-app
...
build-docker-arm64: require-version
	...
	cd ${DOCKER_DIR} && \
		...
		-t ghcr.io/unafraid/gotify/server-arm64:latest \
		-t ghcr.io/unafraid/gotify/server-arm64:${VERSION} \
		-t ghcr.io/unafraid/gotify/server-arm64:$(shell echo $(VERSION) | cut -d '.' -f -2) \
		-t ghcr.io/unafraid/gotify/server-arm64:$(shell echo $(VERSION) | cut -d '.' -f -1) .
	rm ${DOCKER_DIR}gotify-app
...
build-docker-riscv64: require-version
	...
	cd ${DOCKER_DIR} && \
		...
		-t ghcr.io/unafraid/gotify/server-riscv64:latest \
		-t ghcr.io/unafraid/gotify/server-riscv64:${VERSION} \
		-t ghcr.io/unafraid/gotify/server-riscv64:$(shell echo $(VERSION) | cut -d '.' -f -2) \
		-t ghcr.io/unafraid/gotify/server-riscv64:$(shell echo $(VERSION) | cut -d '.' -f -1) .
	rm ${DOCKER_DIR}gotify-app
...
build: build-linux-arm-7 build-linux-amd64 build-linux-arm64 build-linux-riscv64 build-windows-amd64
